...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Run: python run.py -d dataname -r resolution -q QP ")
    parser.add_argument('-d', '--dataname', type=str, required=True, help='dataname')
    parser.add_argument('-r', '--resolution', type=str, required=True, help='resolution')
    parser.add_argument('-q', '--qp', type=str, required=True, help='QP')
...
def run(args):
    ...
    raw_path = '../../data/' + dataname + '/rgb_crop444/'  #测试集
    gt_path = '../../data/' + dataname + '/images/'  #原图集
    rec_path = './results/' + dataname + '/rec/RA' + (QP) +'/' #压缩后的yuv存放路径
    png_path = './results/' + dataname + '/png/RA' + (QP) +'/' #分帧后的png存放路径
    bin_path = './results/' + dataname + '/bin/RA' + (QP) +'/' #编码后的码流存放路径
    log_path = './results/' + dataname + '/log/RA' + (QP) +'/' #编码后的log存放路径
    ...
    for i in range(len(raw_list)):
        ...
        print(yuv_name)
        # os.system('./bin/EncoderAppStatic -c ./cfg/per-class/' + dataname + '.cfg -c ./cfg/encoder_randomaccess_vtm.cfg -q ' + (QP) + ' -i ' + raw_yuvpath + ' -b ' + bin_path_final + '.bin' + ' > '+ log_path_final +  '.txt')
        # os.system('./bin/DecoderAppStatic'  + ' -b ' + bin_path_final + '.bin' + ' -o ' + rec_path_final  + ' > '+ log_path_final +  '.log')
    ...
    for k in range(len(rec_list)):
        ...
        yuv_name = raw_list[k].split('.')[0]
        yuv_num = yuv_name.split('_')[0]
        saveroot = png_path + yuv_num
        savepath = saveroot + '/im%03d.png'
        if not os.path.exists(saveroot):
            os.makedirs(saveroot)
        os.system('ffmpeg -y -pix_fmt yuv444p10le -s ' + resolution + ' -i ' + rec_yuvpath +  ' ' + savepath)
